...
export class BookingService {
 ...
 async generateTicket(bookingId: string): Promise<void> {
  ...
  const agent = await this.AgentRepository.findOneBy({ agentId: agentId });
  ...
  const vat =  booking.itenary.GrossFare * 0.003;
  const type = booking.itenary.PriceBreakDown?.[0]?.PaxType === "ADT" ? 'Adult' : 'Child';
